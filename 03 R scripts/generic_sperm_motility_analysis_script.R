...
# Save overall mean sperm motility data
write.csv(general_mean_SM, 
          file = "02 clean data/general_mean_sperm_motility.csv", 
          row.names = FALSE)

# Save mean sperm motility data
write.csv(means_SM, 
          file = "02 clean data/individual_means_sperm_motility.csv", 
          row.names = FALSE)
...
# Save the mean sperm motility within male plot
ggsave("04 R plots/mean_sperm_motility_within_male.png", width = 10, height = 6)
...
plot(simulation_output_lm)
testResiduals(simulation_output_lm)

run_dharma_tests(simulation_output_lm)

cat("If you get a warning message, refine your model or consult a statistician. 
    Although it might already be to late!")
...
write.csv(model_summary, file = "05 R reports/model_summary.csv", row.names = FALSE)
...
write.csv(emm_df, file = "05 R reports/emmeans_results.csv", row.names = FALSE)

# Calculate and save pairwise comparisons within each time of measurement
simp <- pairs(emm, simple = treatment_var)
write.csv(as.data.frame(simp), file = "05 R reports/pairwise_comparisons.csv", 
          row.names = FALSE)
...
# Save the emmeans plot
ggsave("04 R plots/emmeans_plot.png", width = 10, height = 6)
